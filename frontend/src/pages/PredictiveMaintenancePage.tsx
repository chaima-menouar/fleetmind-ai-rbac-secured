    ...
    <div className="page">
      <div className="page-heading">
        <div>
          ...
          <p>
            FleetMind analyzes vehicle sensor signals and turns them into a clear maintenance priority.
          </p>
        </div>
              ...
              {prediction ? (
                <div className={`prediction-result risk-${prediction.risk_level}`}>
                  <div className="score-ring" style={{ "--score": `${Math.min(prediction.aps_failure_score * 360, 360)}deg` } as React.CSSProperties}>
                    <span><strong>{percent(prediction.aps_failure_score)}</strong><small>risk score</small></span>
                  </div>
                  <div className="prediction-copy">
                    ...
                    <h3>{prediction.predicted_label === "aps_failure" ? "Service recommended" : "No APS service needed"}</h3>
                    <p>Confirmed outcome: <strong>{labelText(prediction.actual_label)}</strong></p>
                    <small>Decision powered by {prediction.model_version}</small>
                  </div>
                </div>
              ) : (
