...
def _vehicle_snapshot(vehicle: VehicleResponse, role: str) -> str:
    core = (
        f"{vehicle.id} ({vehicle.model}) is in {vehicle.location}. "
        f"Status: {vehicle.status}. Health: {vehicle.health_score}%. "
        f"Battery: {vehicle.battery_percent}%. "
        f"Service is {_service_label(vehicle.next_service_days)}."
    )
    if role == "technician":
        return (
            core
            + " Use these observed fields as the starting point, then verify active alerts "
            "and service history before selecting a repair procedure."
        )
    if role == "manager":
        return (
            core
            + " Use this as an operational signal for availability, maintenance scheduling, "
            "and resource prioritization rather than as a remote diagnosis."
        )
    return (
        core
        + " This is a read-only explanation; operational changes must be handled by an "
        "authorized technician or manager."
    )
...
def _demo_completion(
    ...
    if mentioned is not None:
        ...
        core = (
            f"{vehicle.id} has the lowest battery level at {vehicle.battery_percent}% "
            f"with a health score of {vehicle.health_score}%. Its status is "
            f"{vehicle.status}, with service {_service_label(vehicle.next_service_days)}."
        )
        if role == "technician":
            guidance = (
                core
                + " Review charging history, battery alerts, thermal events, and service "
                "diagnostics before return to service."
            )
        elif role == "manager":
            guidance = (
                core
                + " Treat it as an availability risk and prioritize capacity coverage while "
                "maintenance validates the vehicle."
            )
        else:
            guidance = (
                core
                + " A technician should review the vehicle before any operational action "
                "is taken."
            )
    elif (
        any(word in lowered for word in ("maintenance", "service", "alert", "fault", "risk"))
        and fleet
    ):
        due = sorted(
        ...
        if due:
            ...
            if role == "technician":
                guidance += (
                    f"\n\nStart technical triage with {due[0].id}; verify alerts and service "
                    "history before choosing a repair path."
                )
            elif role == "manager":
                guidance += (
                    f"\n\nPrioritize {due[0].id} first and plan coverage by location to protect "
                    "fleet availability."
                )
            else:
                guidance += (
                    "\n\nThese are read-only status signals; maintenance decisions belong to "
                    "the technician or manager role."
                )
        else:
            guidance = "No visible vehicles have service due within the next seven days."
    elif (
        any(word in lowered for word in ("fleet", "vehicle", "availability", "kpi", "summary"))
        and fleet
    ):
        ...
        guidance = (
            "Give me a vehicle ID, fleet KPI, maintenance question, battery concern, "
            "or time window. I will answer using FleetMind's verified fleet data and "
        ...
        "system": [
            {
                "text": (
                    system_prompt
                    + " Use verified context for fleet-specific facts. Never invent telemetry, "
                    "service status, or operational events. Clearly say when the supplied "
